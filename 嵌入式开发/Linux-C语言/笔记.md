...
指针和数组为什么关系这么大呢？最根本的原因是数组在内存中是连续存放的，这一点是其他的任何数据类型都难以比拟的。又因为指针可以进行一些加减运算，其副作用就是指向的地址加减一个数据类型的大小，这样就可以很方便地调用数组了，因此二者的联系相当紧密。其次就是指针的关系运算可以比较的一个前提就是，指针指向的是一个连续的存储空间，这样指针的指向才能够有意义。
